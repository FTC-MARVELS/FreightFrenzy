...
public class Detector implements ImageReader.OnImageAvailableListener, Camera.PreviewCallback {
    ...
    private static final Size DESIRED_PREVIEW_SIZE = new Size(640, 480);// new Size(765, 1070);new Size(640, 480);
    ...
    protected void setFragment() {
        if (useCamera2API) {
            Log.d(TAG, "useCamera2API. IF BLOCK");
            ...
            Log.d(TAG, "useCamera2API. ELSE BLOCK ROTATION");
        }


        ((Activity)appContext).getFragmentManager().beginTransaction().replace(tfodMonitorViewId, fragment).commit();
    //    Log.d(TAG, "FRAGMENT ROTATION" + fragment.getActivity().getWindowManager().getDefaultDisplay().getRotation());
    ...
    protected int getScreenOrientation() {
        Log.d(TAG, "SCREEN ORIENTATION: " + ((Activity)appContext).getWindowManager().getDefaultDisplay().getRotation());
    ...
    public void onPreviewFrame(final byte[] bytes, final Camera camera) {
        ...
        try {
            ...
            if (rgbBytes == null) {
                ...
                Log.d(TAG, "ON PREVIEW FRAME Height and Width : " + previewHeight + ":" + previewWidth);
    ...
    public void onPreviewSizeChosen(final Size size, final int rotation) {
        ...
        previewWidth = size.getWidth();
        previewHeight = size.getHeight();
        Log.e(TAG,"Preview Size: " +previewHeight + ":" + previewWidth);
        Log.e(TAG,"INCOMING ROTATION: " +rotation);

        sensorOrientation = rotation - getScreenOrientation();
    //    sensorOrientation = 180;
        Log.d(TAG, String.format("onPreviewSizeChosen. Camera orientation relative to screen canvas: %d", sensorOrientation) + " Rotation: " + rotation + "SCREEN ORIENTATION: " + getScreenOrientation());
...
//        final int cropSize = Math.min(previewWidth, previewHeight);
        ...
        runInBackground(
                new Runnable() {
                    ...
                    public void run() {
                        if (classifier != null) {
                            lastResults = classifier.recognizeImage(rgbFrameBitmap, 0);
                        }
                        else{
    ...
    private void recreateClassifier(Model model, Device device, int numThreads) {
        ...
        if (classifier != null) {
            ...
            Log.d(TAG, "Classifier done: Image Size" + imageSizeX + ": " + imageSizeY);
        }
        else{
